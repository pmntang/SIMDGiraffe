...
export function instructionsByRegisterBySteps (arrayOfObject) { // objects are quatriples {id, intrinsic, line, registers}
  ...
  arrayOfObject.map(function (anIntrObj) {
    for (let i = 0; i < anIntrObj.registers.length; i++) {
      ...
      const objInstr = { id: anIntrObj.id, intrinsic: anIntrObj.intrinsic, line: anIntrObj.line, idr: anIntrObj.registers.length - i }
      const register = anIntrObj.registers[i]
  ...
  const sorted = newArray.sort((a, b) => (a.register > b.register ? 1 : a.register < b.register ? -1 : 0))
  return sorted
}
...
export function displayWindow (aWindow) { // aWindow is an object {id:, idr:, tab}. This function construct an object {id:, idr:, name:, register:} based on aWindow
  const objectTodisplay = {}; let currentWindow = null
...
export function message (aWindow) { // this function generate a message in the form of and object {head:, body}. the body contains a html element containing a text to display. The text is construct based on aWindow. the property head contains the name of the correspondant register.
  ...
  const messageContent = displayWindow(aWindow)// Below we identify the paragraph (tag p) with the id of aWindow messageContent.id as we will use this id to set the same color for the paragraph having the same id
  const message = { head: messageContent.register, body: <p className={messageContent.id} ><span style={styleText}>instruction:</span> <span style={styleId}>{messageContent.id}</span><br /><span style={styleText}>step:</span> <span style={styleId}>{messageContent.idr}</span><br /><span style={styleId}>{messageContent.name}</span></p> }
  return message
}
...
export function buildMatrixRegInt (anArrayOfRegisters, anArrayOfInstructions) {
  const augMentArrayOfReg = [{ register: 'NAME' }, ...anArrayOfRegisters]
...
export function advanceMatrixWindow (lineIndex, columnIndex, aMatrix) { // Advance the window to the next step of an instruction or until the next instruction.
  return {
    value: searchStep(lineIndex, columnIndex + 1, aMatrix)
      ? aMatrix[lineIndex][columnIndex + 1]
      : ((lineIndex >= aMatrix.length - 1 && columnIndex >= aMatrix[0].length - 1 && searchStep(1, 0, aMatrix))
          ? aMatrix[1][0]
          : (((lineIndex >= aMatrix.length - 1 && columnIndex >= aMatrix[0].length - 1) || (lineIndex <= 0 || columnIndex < 0))
              ? advanceMatrixWindow(1, 0, aMatrix).value
              : ((columnIndex >= aMatrix[0].length - 1 && searchStep(lineIndex + 1, 0, aMatrix))
                  ? aMatrix[lineIndex + 1][0]
                  : ((columnIndex >= aMatrix[0].length - 1 && lineIndex < aMatrix.length - 1)
                      ? advanceMatrixWindow(lineIndex + 1, 0, aMatrix).value
                      : ((lineIndex >= aMatrix.length - 1 && searchStep(aMatrix.length - 1, columnIndex + 1, aMatrix))
                          ? aMatrix[aMatrix.length - 1][columnIndex + 1]
                          : (advanceMatrixWindow(lineIndex, columnIndex + 1, aMatrix).value)))))),
    indexes: searchStep(lineIndex, columnIndex + 1, aMatrix)
      ? [lineIndex, columnIndex + 1]
      : ((lineIndex >= aMatrix.length - 1 && columnIndex >= aMatrix[0].length - 1 && searchStep(1, 0, aMatrix))
          ? [1, 0]
          : (((lineIndex >= aMatrix.length - 1 && columnIndex >= aMatrix[0].length - 1) || (lineIndex <= 0 || columnIndex < 0))
              ? advanceMatrixWindow(1, 0, aMatrix).indexes
              : ((columnIndex >= aMatrix[0].length - 1 && searchStep(lineIndex + 1, 0, aMatrix))
                  ? [lineIndex + 1, 0]
                  : ((columnIndex >= aMatrix[0].length - 1 && lineIndex < aMatrix.length - 1)
                      ? advanceMatrixWindow(lineIndex + 1, 0, aMatrix).indexes
                      : ((lineIndex >= aMatrix.length - 1 && searchStep(aMatrix.length - 1, columnIndex + 1, aMatrix))
                          ? [aMatrix.length - 1, columnIndex + 1]
...
export function initializeLinesMatrix (aLine, aMatrix) {
  let ligne1 = <th rowSpan="3" scope="rowgroup" className="empty"></th>; let ligne2 = null; let ligne3 = null
...
export function initializeFirstLineMatrix (firstLine, aMatrix) { // should have call this initializeFirstLigneMatrix
  let ligne1 = <th rowSpan="3" scope="rowgroup" className="intrinsicName">{aMatrix[firstLine][0].name.toUpperCase()}</th>; let ligne2 = null; let ligne3 = null
...
export function retrieveIndexPositionLine (aLine, aColumn, aPosition, aMatrix) { // all the positions before aPosition (those wich position are <= aPosition.rank) are retrieve
  ...
  if (aLine == aPosition.line) { // this condition could have been left
    const indexPosition = buildPosition(aLine, aColumn, aMatrix)
    if (indexPosition) {
      const l = indexPosition.length
      switch (l) {
...
export function buildNonNulPositionsLine (indexLine, aMatrix) {
  const positionsLine = aMatrix[indexLine].map((e, i) => searchStep(indexLine, i, aMatrix) ? buildPosition(indexLine, i, aMatrix) : null).flat().filter(e => e).sort((a, b) => a.rank - b.rank)
...
export function advancePosition (aPosition, aMatrix) {
  return buildNonNulPositionsLine(aPosition.line, aMatrix).find(e => e.rank > aPosition.rank)
    ? buildNonNulPositionsLine(aPosition.line, aMatrix).find(e => e.rank > aPosition.rank)
...
export function advanceLinePosition (aPosition, aMatrix) {
  return (aPosition.line === advancePosition(aPosition, aMatrix).line && aPosition.rank !== null)
    ? advancePosition(aPosition, aMatrix)
...
export function nextPositions (aPosition, aMatrix) {
  let pathNextpos = []
  if (aPosition.rank > 0) {
    let forbidenColumn = 0
    pathNextpos = [aPosition, ...pathNextpos]
    const lastPositionLine = buildNonNulPositionsLine(aPosition.line, aMatrix)[buildNonNulPositionsLine(aPosition.line, aMatrix).length - 1]
    ...
    for (let i = aPosition.line; i < aMatrix.length; i++) {
      const positionsLine = buildNonNulPositionsLine(i, aMatrix)
      const posNext = positionsLine.find(e => e.column != forbidenColumn && e.column == aPosition.column && e.line != aPosition.line && !pathNextpos.find(x => _.isEqual(x, e)))
      if (posNext) {
...
export function previousPositions (aPosition, aMatrix) {
  let pathNextpos = []
  if (aPosition.rank > 0) {
    const authorizedColumn = aPosition.column // the previous position if not on the same line (wich is always the case for an out position) is always on the same column as the position
    pathNextpos = [aPosition, ...pathNextpos]
    const lastPositionLine = buildNonNulPositionsLine(aPosition.line, aMatrix)// [buildNonNulPositionsLine(aPosition.line, aMatrix).length-1]
    if (aPosition.rank == lastPositionLine[lastPositionLine.length - 1].rank) { //  all the positions of the same line are always the predecessor of the out position
      ...
      for (let i = aPosition.line; i > 0; i--) {
        const positionsLine = buildNonNulPositionsLine(i, aMatrix)
        const posNext = positionsLine.find(e => e.column == authorizedColumn && e.line != aPosition.line && !pathNextpos.find(x => _.isEqual(x, e)))
        if (posNext) {
...
export function forwardPathPosition (aPosition, aMatrix) {
  let forwardPathOfPosition = nextPositions(aPosition, aMatrix)
  let level = 1
...
export function backwardPathPosition (aPosition, aMatrix) {
  let backwardPathOfPosition = []
  for (let i = 1; i < aPosition.line; i++) {
    const positionsLine = buildNonNulPositionsLine(i, aMatrix)
    for (let j = 0; j < positionsLine.length; j++) {
      const forwardPathOfPositionj = forwardPathPosition(positionsLine[j], aMatrix)
  ...
  const positionsLine = buildNonNulPositionsLine(aPosition.line, aMatrix)
...
export function extractPositionFromId (aGivenId) {
  const positionPropertiesArray = aGivenId.split(/[lcrz]/)
...
export function maxPosition (aMatrix) {
  const pos = buildNonNulPositionsLine(aMatrix.length - 1, aMatrix)[buildNonNulPositionsLine(aMatrix.length - 1, aMatrix).length - 1]
  return pos
}
...
export function updateArrayOfCurrentPositions (anArrayOfCurrentPositions, idOfEventElt, aMatrix) {
  const thisPosition = extractPositionFromId(idOfEventElt)// ; console.log("nexposition",  nextPositions(thisPosition, aMatrix), "previous", previousPositions(thisPosition, aMatrix))
  const eltId = anArrayOfCurrentPositions.find(e => e.anElementId == idOfEventElt) // [...aListOfCurrentPosition, {aPosition:pathElt[0], anElementId:idOfEventElt}]
  if (!eltId) {
    const pathElt = consPath(extractPositionFromId(idOfEventElt), aMatrix)
    const pathEltDown = forwardPathPosition(extractPositionFromId(idOfEventElt), aMatrix)
    const pathEltUp = backwardPathPosition(extractPositionFromId(idOfEventElt), aMatrix).concat(thisPosition)
    const positionsOfThisLine = buildNonNulPositionsLine(pathElt[0].line, aMatrix).filter(e => e.rank >= 1)
    const positionsOfThisLineUp = buildNonNulPositionsLine(pathEltUp[0].line, aMatrix).filter(e => e.rank >= 1)
    const positionsOfThisLineDown = buildNonNulPositionsLine(pathEltDown[0].line, aMatrix).filter(e => e.rank >= 1)
    const restrictPositions = linePosition(pathElt, pathElt[0], aMatrix).filter(e => !_.isEqual(e, thisPosition)).length == 0 ? [positionsOfThisLine[positionsOfThisLine.length - 1]] : linePosition(pathElt, pathElt[0], aMatrix).filter(e => !_.isEqual(e, thisPosition))
    const restrictPositionsup = linePosition(pathEltUp, pathEltUp[0], aMatrix).filter(e => !_.isEqual(e, thisPosition)).length == 0 ? [positionsOfThisLineUp[positionsOfThisLineUp.length - 1]] : linePosition(pathEltUp, pathEltUp[0], aMatrix).filter(e => !_.isEqual(e, thisPosition))
    const restrictPositionsDown = linePosition(pathEltDown, pathEltDown[0], aMatrix).filter(e => !_.isEqual(e, thisPosition)).length == 0 ? [positionsOfThisLineDown[positionsOfThisLineDown.length - 1]] : linePosition(pathEltDown, pathEltDown[0], aMatrix).filter(e => !_.isEqual(e, thisPosition))
    anArrayOfCurrentPositions = [...anArrayOfCurrentPositions, {
      aCurrentPosition: restrictPositions,
      aCurrentPositionUp: previousPositions(thisPosition, aMatrix),
      aCurrentPositionDown: nextPositions(thisPosition, aMatrix),
      anElementId: idOfEventElt,
      idPosition: minFreePosition(anArrayOfCurrentPositions),
      listOfPath: pathElt,
      listOfPathUp: pathEltUp,
      listOfPathDown: pathEltDown,
      linkedPositionsUp: [thisPosition],
      linkedPositionsDown: [thisPosition]
    }]
  } else {
...
export function advanceAselectPosition (anObjectOfCurrentPosition, aMatrix) {
  const thisPosition = extractPositionFromId(anObjectOfCurrentPosition.anElementId)
  const linePositions = buildNonNulPositionsLine(anObjectOfCurrentPosition.listOfPath[0].line, aMatrix)
  const firstPosition = linePosition(anObjectOfCurrentPosition.listOfPath, anObjectOfCurrentPosition.listOfPath[0], aMatrix).filter(e => !_.isEqual(e, thisPosition)).length == 0
    ? [linePositions[linePositions.length - 1]]
    : linePosition(anObjectOfCurrentPosition.listOfPath, anObjectOfCurrentPosition.listOfPath[0], aMatrix).filter(e => !_.isEqual(e, thisPosition))
  ...
  if (_.isEqual(anObjectOfCurrentPosition.aCurrentPosition[anObjectOfCurrentPosition.aCurrentPosition.length - 1], anObjectOfCurrentPosition.listOfPath[anObjectOfCurrentPosition.listOfPath.length - 1])) {
    ...
    const anextPosition = anObjectOfCurrentPosition.listOfPath.find(e => (e.line > anObjectOfCurrentPosition.aCurrentPosition[0].line || (e.line == anObjectOfCurrentPosition.aCurrentPosition[0].line && e.rank > anObjectOfCurrentPosition.aCurrentPosition[0].rank)) && !anObjectOfCurrentPosition.aCurrentPosition.some(x => _.isEqual(x, e)))
    const nextPositions = linePosition(anObjectOfCurrentPosition.listOfPath, anextPosition, aMatrix).filter(e => !_.isEqual(e, thisPosition))
    if (nextPositions.length > 0) {
      anObjectOfCurrentPosition.aCurrentPosition = nextPositions
    } else {
      const followingPosition = anObjectOfCurrentPosition.listOfPath.find(e => (e.line > thisPosition.line || (e.line == thisPosition.line && e.rank > thisPosition.rank)) && !linePosition(anObjectOfCurrentPosition.listOfPath, thisPosition, aMatrix).some(x => _.isEqual(x, e)))
...
export function advanceAselectPositionFoward (anObjectOfCurrentPosition, aMatrix) {
  const thisPosition = extractPositionFromId(anObjectOfCurrentPosition.anElementId)
  const linePositions = buildNonNulPositionsLine(anObjectOfCurrentPosition.listOfPathDown[0].line, aMatrix)
  const firstPosition = linePosition(anObjectOfCurrentPosition.listOfPathDown, anObjectOfCurrentPosition.listOfPathDown[0], aMatrix).filter(e => !_.isEqual(e, thisPosition)).length == 0
    ? [linePositions[linePositions.length - 1]]
    : linePosition(anObjectOfCurrentPosition.listOfPathDown, anObjectOfCurrentPosition.listOfPathDown[0], aMatrix).filter(e => !_.isEqual(e, thisPosition))
  ...
  if (_.isEqual(anObjectOfCurrentPosition.aCurrentPositionDown[anObjectOfCurrentPosition.aCurrentPositionDown.length - 1], anObjectOfCurrentPosition.listOfPathDown[anObjectOfCurrentPosition.listOfPathDown.length - 1])) {
    ...
    const anextPosition = anObjectOfCurrentPosition.listOfPathDown.find(e => (e.line > anObjectOfCurrentPosition.aCurrentPositionDown[0].line || (e.line == anObjectOfCurrentPosition.aCurrentPositionDown[0].line && e.rank > anObjectOfCurrentPosition.aCurrentPositionDown[0].rank)) && !anObjectOfCurrentPosition.aCurrentPositionDown.some(x => _.isEqual(x, e)))
    const nextPositions = linePosition(anObjectOfCurrentPosition.listOfPathDown, anextPosition, aMatrix).filter(e => !_.isEqual(e, thisPosition))
    if (nextPositions.length > 0) {
      anObjectOfCurrentPosition.aCurrentPositionDown = nextPositions
    } else {
      const followingPosition = anObjectOfCurrentPosition.listOfPathDown.find(e => (e.line > thisPosition.line || (e.line == thisPosition.line && e.rank > thisPosition.rank)) && !linePosition(anObjectOfCurrentPosition.listOfPathDown, thisPosition, aMatrix).some(x => _.isEqual(x, e)))
      if (followingPosition) {
...
export function findPositionInCurrentPositions (indexLine, aListOfCurrentPosition) {
  return aListOfCurrentPosition.find(e => e.aPosition.line == indexLine || extractPositionFromId(e.anElementId).line == indexLine)
    ? (aListOfCurrentPosition.find(e => e.aPosition.line == indexLine)
        ? aListOfCurrentPosition.find(e => e.aPosition.line == indexLine).aPosition
        : extractPositionFromId(aListOfCurrentPosition.find(e => extractPositionFromId(e.anElementId).line == indexLine).anElementId))
    : null
}

export function matrixToPosition (aMatrix) { // build a matrix of position with a given matrix
  const matrixPosition = aMatrix.map((e, i) => i == 0
    ? e
    : e.map((x, j) => j == 0
      ? x
      : (buildPosition(i, j, aMatrix)
          ? (buildPosition(i, j, aMatrix).length == 1
              ? (_.isEqual(buildPosition(i, j, aMatrix)[0], buildNonNulPositionsLine(i, aMatrix)[buildNonNulPositionsLine(i, aMatrix).length - 1])
                  ? Array.of(Array.of(), Array.of(), Array.of(buildPosition(i, j, aMatrix)[0]))
...
export function emptyPositionsAndCoordinateOfFigures (aMatrixPosition, aMatrixCoordinate) { // build a matrix of figures (svg elements) with a given matrix of positions and a matrix of coordinates (with empty classNames)
  const emptyMatrixFigures = aMatrixPosition.map((e, i) => i == 0
    ? e.map((x, j) => j == 0
      ? (<g transform={'translate(' + aMatrixCoordinate[i][j][0] + ',' + aMatrixCoordinate[i][j][1] + ')'}><rect className="name" x="0" y="0" width={'' + aMatrixCoordinate[i][j][2]} height={'' + aMatrixCoordinate[i][j][3]}></rect>
    ...
    : e.map((x, j) => j == 0
      ? (<g transform={'translate(' + aMatrixCoordinate[i][j][0] + ',' + aMatrixCoordinate[i][j][1] + ')'}><rect className="emptyIntrinsicName" x="0" y="0" width={'' + aMatrixCoordinate[i][j][2]} height={'' + aMatrixCoordinate[i][j][3]}></rect>
...
export function matrixToCoordinate (aMatrx, anOrigin, widthOfFigures, heightOfFigures) { // build a matrix of coordinates with a given matrix, origin, width and height of figures
  const matrixCoordinate = aMatrx.map((e, i) => i == 0
    ? e.map((x, j) => Array.of(anOrigin + j * widthOfFigures, anOrigin + i * heightOfFigures, widthOfFigures, heightOfFigures))
    : e.map((x, j) => j == 0
      ? Array.of(anOrigin + j * widthOfFigures, anOrigin + i * heightOfFigures, widthOfFigures, heightOfFigures)
...
export function constructPositionFromId (anIdOfFirstColumnPosition) {
  const myArray = /(\d+)name\S*line(\d+)/g.exec(anIdOfFirstColumnPosition)
  const line = parseInt(myArray[1], 10)
  const codeLine = parseInt(myArray[2], 10)
...
export function constructDescription (aSimdFunction, aDescriptionFile, returnType) { // take a name of a function (which return returnType) and construct a look-like manufacturer description using a file provide by the manufacturer (Intel)
  ...
  const parameter = !objectDescription.hasOwnProperty('parameter') ? 'void' : objectDescription.parameter
  objectDescription = { ...objectDescription, parameter }// we add parameter property if it doesn't yet exist
  const line1 = `${objectDescription._rettype} ${objectDescription._name} (${Array.isArray(objectDescription.parameter) ? objectDescription.parameter.flatMap((e, i) => i < objectDescription.parameter.length - 1 ? [e._type, e._varname + ','] : [e._type, e._varname]).join(' ') : objectDescription.parameter})`
  const line2 = `Synopsis \n\t\t${line1} \n\t\t#include <${objectDescription.header}>\n\t\tInstruction: ${objectDescription.hasOwnProperty('instruction') ? objectDescription.instruction[0]._name + ' ' + objectDescription.instruction[0]._form : 'sequence'}\n\t\t${objectDescription.hasOwnProperty('CPUID') ? 'CPUID Flags: ' + objectDescription.CPUID : ''} `
  const line3 = `Description \n\t\t${objectDescription.description}`
  const line4 = `Operation \n\t\t${objectDescription.operation.replaceAll('\t', '\t\t').replaceAll('\n', '\n\t\t')}`
  return [objectDescription, `${line1}\n\n\t${line2}\n\n\t${line3}\n\n\t${line4}`]
}

export function constructDataListTable (aSimdDescriptionFile) { // take a file provide the manufacturer (like Intel) where simd instructions are describe and return a table with the name of simd instructions on other features
  return aSimdDescriptionFile.intrinsic.map(e => {
    const parameter = !e.hasOwnProperty('parameter') ? 'void' : e.parameter
    let objectDescription = e
    objectDescription = { ...objectDescription, parameter }// we add parameter property if it doesn't yet exist
    const line1 = `${objectDescription._rettype} ${objectDescription._name} (${Array.isArray(objectDescription.parameter) ? objectDescription.parameter.flatMap((e, i) => i < objectDescription.parameter.length - 1 ? [e._type, e._varname + ','] : [e._type, e._varname]).join(' ') : objectDescription.parameter})`
    const line2 = `Synopsis \n\t\t${line1} \n\t\t#include <${objectDescription.header}>\n\t\tInstruction: ${objectDescription.hasOwnProperty('instruction') ? objectDescription.instruction[0]._name + ' ' + objectDescription.instruction[0]._form : 'sequence'}\n\t\t${objectDescription.hasOwnProperty('CPUID') ? 'CPUID Flags: ' + objectDescription.CPUID : ''} `
    const line3 = `Description \n\t\t${objectDescription.description}`
    const line4 = typeof (objectDescription.operation) === 'string' ? `Operation \n\t\t${objectDescription.operation.replaceAll('\t', '\t\t').replaceAll('\n', '\n\t\t')}` : 'Operation \n\t\t No operation'
...
export function computeOperandsAndresultElt (aSimdDescriptionFile) { // take a file provide the manufacturer (like Intel) where simd instructions are describe and return and array of objects {name:"nameOfIntrinsicInstruction", retype:"returnType",operands:[], result: [], varnames:[], types:[], sizeOfScalarFiel:0} desscribing each instruction
  return aSimdDescriptionFile.intrinsic.map(e => {
    ...
    const name = e._name
    obj = { ...obj, name }
    const regTypeNumber = /(?<=_m|__m|_int|__int)\d+/g
    const regextractSizeFromDesc = /\d+(?=-bit)/g// We have notice that the size of scalar unit of a vector instruction can be assimilated to a x-bit expression in the description of tha instruction
    const arrayOfMinimunSizeInDescription = e.description.match(regextractSizeFromDesc)
    const sizeOfScalarField = arrayOfMinimunSizeInDescription ? Math.min(...arrayOfMinimunSizeInDescription) : 1
    let maxSize = sizeOfScalarField
    obj = { ...obj, sizeOfScalarField }
    let result = []
    if (e._rettype) {
      const retType = e._rettype
      obj = { ...obj, retType }
      ...
      if (retTypeNumber != 0) {
        const size = retTypeNumber / sizeOfScalarField
    ...
    let operands = []; const varnames = []; const types = []
    if (e.parameter && Array.isArray(e.parameter) && !e.parameter.find(e => e._varname === '' || e._type === 'void')) {
      e.parameter.forEach((element, k) => {
        if (element._type) {
          ...
          const operandNumber = types[k].match(regTypeNumber)
          let operand = []
          if (operandNumber) {
            const operandSize = operandNumber[0]
            maxSize = operandSize > maxSize ? operandSize : maxSize
            const size = operandSize / sizeOfScalarField
            for (let i = 0; i < size; i++) {
              operand = [...operand, i]
            }
            ...
            const fieldArray = types[k].match(/(?<=_m|__m|_int|__int)\w*mask\w*/g)
            const operandSize = (fieldArray || element._varname == 'imm8') ? maxSize : sizeOfScalarField
            const size = operandSize / sizeOfScalarField
            for (let i = 0; i < size; i++) {
              operand = [...operand, i]
            }
    ...
    if (result.length == 0 && e._rettype.match(/(?<=_m|__m|_int|__int)\w*mask\w*/g)) {
      const size = maxSize / sizeOfScalarField
      for (let i = 0; i < size; i++) {
        result = [...result, i]
      }
      obj = { ...obj, result }
...
export function insertOperand (opName, opType, opRank, opDimension, aLinkingObject) {
  if (opName === 'r') {
    const result = opDimension == 0 ? range(0, opDimension, opDimension) : range(0, opDimension - 1, 1)
    const retType = opType
...
export function buildMessage (operators, operands) {
  ...
  const message = globalTab.sort((o1, o2) => o1.rank - o2.rank).map(o => {
    ...
    const f = (match, p1, p2, offset, string) => {
      ...
      groupedMatch = allMatch.reduce((acc, crv, cri, arr) => {
        if (cri === 0 || !acc.find(e => e.some(x => x[1] === crv[1]))) {
          ...
          const indexGroup = acc.findIndex(e => e.some(x => x[1] === crv[1]))
...
export function readLinkingIndexMsg (aLinkingIndex) { // take a linking index and return a message
  let Msge = ''
  const indexOfactive = aLinkingIndex.findIndex(e => e[0] == 'active') // console.log("aLinkingIndex", aLinkingIndex);
...
export function readAllLinkinIndex (aLinkingIndex, aResultTable) {
  const result = aResultTable.map((e, i) => e.state === 'active' ? ({ state: aLinkingIndex[i].slice(1).flat().sort((a, b) => a[1] - b[1]).reduce((pre, cur) => cur[2] ? [...pre, cur[2], cur[0]] : [...pre, cur[0]], []).join(' '), fieldResult: e.fieldResult }) : e)
