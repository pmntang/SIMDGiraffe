...
const functionNameIfNoFound=/(\w*(?=\s*\())/; //Here we capture a fonction name if the first attempt fails.This is to process the case where the returned compiled code begin with constants (see interleave_uint8_with_zeros_avx_lut)

function findLineHeadIndex(arawAsm){//return the index of the line containing the name of the function
    return arawAsm.findIndex(e=>functionNameIfNoFound.test(e.text))
}


export function generateASM(rawAsm) {
    rawAsm=rawAsm.splice(findLineHeadIndex(rawAsm))
    let asm = [];
    let currentFunction = {
        body: []
    }
